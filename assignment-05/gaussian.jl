...
export Gaussian1D, Gaussian1DFromMeanVariance, mean, variance, absdiff, logNormProduct, logNormRatio, GaussianND
...
A multivariate Gaussian distribution with a mean vector and a covariance matrix.
"""
struct GaussianND
	mean::Vector{Float64}
	cov::Matrix{Float64}
	GaussianND(mean::Vector{Float64}, cov::Matrix{Float64}) = new(mean, cov)
end

"""
	GaussianND(n::Int)

Create a multivariate Gaussian distribution with a zero mean vector and an identity covariance matrix.
"""
GaussianND(n::Int) = GaussianND(zeros(n), Matrix{Float64}(I, n, n))
