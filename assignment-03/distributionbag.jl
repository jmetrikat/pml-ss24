...
"""
    reset!(db::DistributionBag{T}, indices::Vector{Int})

Takes a list of indices and resets all distributions from that list to the uniform in the distribution bag `db`

"""
function reset!(db::DistributionBag{T}, indices::Vector{Int}) where {T}
    for i in indices
        db.bag[i] = db.uniform
    end
    return
